...
//Apel au paramètres de connexion(connect.php)
             ...
             $requet = "SELECT libPoste, nom, COUNT(*) as nbVote FROM vote v, candidat c, poste p WHERE v.idCandidat = c.idCandidat AND c.idPoste = p.idPoste and p.idPoste=1 GROUP By p.idPoste, c.idCandidat ORDER BY p.idPoste,  nbVote DESC";
             $requet1 = "SELECT  nom FROM candidat WHERE candidat.idCandidat NOT IN (SELECT idCandidat FROM vote) AND candidat.idPoste = 1";
