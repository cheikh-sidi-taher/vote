...
if(isset($_POST['envoyer']))
  {
			     ...
			     echo'<script language="javascript">
					document.location="RelationExterieur.php"
	            </script>';
...
//Apel au paramètres de connexion(connect.php)
             ...
             $requet = "SELECT libPoste, nom, COUNT(*) as nbVote FROM vote v, candidat c, poste p WHERE v.idCandidat = c.idCandidat AND c.idPoste = p.idPoste and p.idPoste=6 GROUP By p.idPoste, c.idCandidat ORDER BY p.idPoste,  nbVote DESC";
             $requet1 = "SELECT  nom FROM candidat WHERE candidat.idCandidat NOT IN (SELECT idCandidat FROM vote) AND candidat.idPoste = 6";
	   ...
	   if($result = mysqli_query($id,$requet) AND $result1 = mysqli_query($id,$requet1))
	     {
		  //Récuperation du résultat de la requête SQL
								  echo"	<table class='static'>
										  <thead>
											<tr>
											  <th width='40%'><h3>prédication الشـــؤون الدعــوية</h3></th>
											  <th width='30%'><h3>les Candidats المــرشحيـــن</h3></th>
											  <th width='30%'><h3>Nombre  العــدد</h3></th>
					...
					?>
								...
								<div class="padding_10">	
									...
									<button align="center" type="submit" name="envoyer" class="button blue" ><img src="../images/icons/white/png/checkmark_icon&16.png" />Suivant</button>
