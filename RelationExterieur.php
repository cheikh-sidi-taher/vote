...
//Apel au paramètres de connexion(connect.php)
             ...
             $requet = "SELECT libPoste, nom, COUNT(*) as nbVote FROM vote v, candidat c, poste p WHERE v.idCandidat = c.idCandidat AND c.idPoste = p.idPoste and p.idPoste=7 GROUP By p.idPoste, c.idCandidat ORDER BY p.idPoste,  nbVote DESC";
             $requet1 = "SELECT  nom FROM candidat WHERE candidat.idCandidat NOT IN (SELECT idCandidat FROM vote) AND candidat.idPoste = 7";
    ...
    <section id="main" class="container_16">
        ...
        <section id="maincontainer"  style="color: #fff; font-size: 20px;">
			...
			<div class="fixed_page content">	
				...
				<h3>charge des relations exterieurs  المسؤول عن العـلاقات الخارجية</h3>
