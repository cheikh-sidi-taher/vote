...
//Apel au paramètres de connexion(connect.php)
             include("connection/connect.php");
             $requeta = "SELECT libPoste, nom, COUNT(*) as nbVote FROM vote v, candidat c, poste p WHERE v.idCandidat = c.idCandidat AND c.idPoste = p.idPoste GROUP By p.idPoste, c.idCandidat ORDER BY p.idPoste,  nbVote DESC";
    ...
    <section id="main" class="container_16">
        ...
        <section id="maincontainer" style="color: #fff; font-size: 20px;">
            ...
            <div id="fixed_page header" >
				...
				<br>
                    <h1 align ="center">Bureau gagnant  
						<span class="tools">
					...
					<div class="block">
						...
						<table class="static">
						  ...
						  <tbody>
							<tr>
							<?php
	   if($result = mysqli_query($id,$requeta))
	     {
							  ...
							  while($ligne = mysqli_fetch_row($result))
		                       {
				                ...
				                $nb        = $ligne[2];
												...
												// 	$postAr='مدقق حسابات';
												//  if($id==5)
												// 	$postAr='المسؤ ول عن الشؤون المـــــــاليــة';
											
												//  if($id==7)
												// 	$postAr='العـــلاقــات الخـــارجيــة';
												//  if($id==8)
												// 	$postAr='الــــــرياضــــــــة';

								
								if($poste ==  $aposte || $aposte =="" )
								{
					...
					?>
						...
						<br><br>
								<div class="padding_10">	
									...
									<button align="center" type="submit" name="envoyer" class="button red"  style="font-size: 30px;"> Vider</button>
